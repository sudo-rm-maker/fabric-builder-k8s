...
// param {[]String} owners The MSPIDs of the owner orgs of the contract who will have priviledge to mint and burn
func (s *SmartContract) Initialize(ctx contractapi.TransactionContextInterface, name string, symbol string, decimals string, owners []string) (bool, error) {
	...
	// Check minter authorization - one can only initialize contract it owns
	...
	// write owner info
	ownerComposite, err := ctx.GetStub().CreateCompositeKey("string", owners)
	if err != nil {
...
// Checks if client is the owner of the contract
