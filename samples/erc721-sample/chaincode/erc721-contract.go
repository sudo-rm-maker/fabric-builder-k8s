...
const ownerListPrefix = "ownerList"

// Define key names for options
const nameKey = "name"
const symbolKey = "symbol"
const ownerCompositeKey = "ownerComposite"
...
func (c *TokenERC721Contract) Initialize(ctx contractapi.TransactionContextInterface, name string, symbol string, owners []string) (bool, error) {
	// Check minter authorization
	clientMSPID, err := ctx.GetClientIdentity().GetMSPID()
	if err != nil {
		return false, fmt.Errorf("failed to get MSPID: %v", err)
	}
	// fmt.Printf("MSPID is: %v", clientMSPID)
	auth := false
	for _, v := range owners {
		if v == clientMSPID {
			fmt.Printf("Client authorized")
			auth = true
		}
	}
	if !auth {
		return false, fmt.Errorf("client is not the owner of the contract")
	}

	// Check if contract options are not already set, client is not authorized to change them once intitialized
	bytes, err := ctx.GetStub().GetState(nameKey)
	if err != nil {
	...
	// write owner info
	ownerComposite, err := ctx.GetStub().CreateCompositeKey(ownerListPrefix, owners)
	if err != nil {
		return false, fmt.Errorf("failed to create composite key for owners: %v", err)
	}

	err = ctx.GetStub().PutState(ownerCompositeKey, []byte(ownerComposite))
	if err != nil {
		return false, fmt.Errorf("failed to set owners: %v", err)
	}
...
func (c *TokenERC721Contract) MintWithTokenURI(ctx contractapi.TransactionContextInterface, tokenId string, tokenURI string) (*Nft, error) {
	...
	// Check minter authorization
	auth, err := checkOwner(ctx)
	if err != nil {
		return nil, fmt.Errorf("failed to check authorization: %v", err)
	}
	if !auth {
		return nil, fmt.Errorf("authorization failedL %v", err)
	}
...
// Checks if clients are owners of the contract when mint/burn
func checkOwner(ctx contractapi.TransactionContextInterface) (bool, error) {
	clientMSPID, err := ctx.GetClientIdentity().GetMSPID()
	if err != nil {
		return false, fmt.Errorf("failed to get client's MSPID: %v", err)
	}

	ownerComposite, err := ctx.GetStub().GetState(ownerCompositeKey)
	if err != nil {
		return false, fmt.Errorf("failed to get owners composite key")
	}

	_, owners, err := ctx.GetStub().SplitCompositeKey(string(ownerComposite))
	if err != nil {
		return false, fmt.Errorf("failed to split owners composite key")
	}
	...
	if !auth {
		return false, fmt.Errorf("client is not authorized")
	} else {
		return true, nil
	}
}
